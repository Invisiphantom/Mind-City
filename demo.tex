...
$\delta|_\sigma:F(a)\to E'$存在$\iff m^\sigma(x)$在$E'$有根, 并且同态数目$=m^\sigma(x)$非重根数
...
![](https://s2.loli.net/2024/08/04/39rY8tPMgxqsukS.png =250x)

------

命题4: 分裂域$\iff$有限正规扩张

$\implies$: 不可约多项式$p(x):\{a,\beta_1,\cdots,\beta_r\}:E/F$
任取分裂域$g(x):L/F$, 现欲证明$a\in L\implies\beta_i\in L$
...
\end{cases}$
由命题2可知, 分裂域上存在开拓域同构 $\delta|_{\eta_i}:L\leftrightarrow L$
由命题3可知, $\delta$是域$L$的F-自同构$\implies\delta|E$是域$E$的F-自同构
