...
将群G中所有元素阶的最小公倍数称为群的指数, 记为$\text{exp}(G)$
...
$\implies$: $G=\langle x\rangle\implies\forall x^k\in G,(x^k)^n=e\land(x^{<n}\neq e)\implies\text{exp}(G)=n=|G|$
...
换位子群的定义(Derived Group)
将所有换位子**生成**的子群 称为群G的换位子群(导群)
记为$G'=\langle xyx^{-1}y^{-1}|x,y\in G\rangle$

命题1: 导群是正规子群, 即$G'\lhd G$
$\forall g\in G,z\in G',gzg^{-1}=(gzg^{-1})(zz^{-1})=(gzg^{-1}z^{-1})(z)\in G'$

命题2: 群$G$为交换群$\iff G'=\{e\}$
命题3: $G/N$为交换群$\iff G'\subseteq N$
证明: $\forall x,y\in G,xN\circ yN=yN\circ xN$
$\iff \forall x,y\in G,xyx^{-1}y^{-1}\in N$
$\iff G'\subseteq N$

示例1: 求4阶交错群$A_4$的换位子群
已知$A_4$的一个正规子群$V=\{e,(12)(34),(13)(24),(14)(23)\}$
由于$|A_4/V|=\frac{12}{4}=3$是素数, 所以$A_4/V$是交换群, 故$A_4'\subseteq V$
...
记群G的一阶导群为$G'=\langle [x,y]|x,y\in G\rangle$
群G的二阶导群为$G^{(2)}=(G')'$, 三阶导群为$G^{(3)}=(G^{(2)})'$
如果存在正整数k, 使得$G^{(k)}=\{e\}$, 则称群G为可解群

<br>

命题1: 群G是可解群, 当且仅当存在递降的子群列
$G=G_0\rhd G_1\rhd G_2\rhd\cdots\rhd G_k=\{e\}$
并且每个商群$G_{i-1}/G_i$都是交换群

$\implies$: 因为群G是可解群, 所以存在导群列$G\rhd G'\rhd G^{(2)}\rhd\cdots\rhd G^{(k)}=\{e\}$
并且由于$G^{i}$是$G^{(i-1)}$的换位子群, 所以$G^{(i-1)}/G^{(i)}$是交换群

$\impliedby$: 用数学归纳法进行证明
$G_0/G_1是交换群\implies G'\subseteq G_1$
$G_1/G_2是交换群\implies G^{(2)}=(G')'\subseteq(G_1)'\subseteq G_2$
$G_2/G_3是交换群\implies G^{(3)}=(G^{(2)})'\subseteq(G_2)'\subseteq G_3$
以此类推, 即可得到$G^{(k)}\subseteq G_k=\{e\}$

<br>

命题2: 可解群的子群都是可解群
$H\subseteq G\implies H'\subseteq G'\implies H^{(k)}\subseteq G^{(k)}=\{e\}$

命题3: 可解群的同态像群都是可解群
$\sigma(G')\subseteq \sigma(G)'$: $\forall[x,y]\in G',\sigma[x,y]=\sigma(xyx^{-1}y^{-1})=\sigma(x)\sigma(y)\sigma(x)^{-1}\sigma(y)^{-1}=[\sigma(x),\sigma(y)]\in \sigma(G)'$
$\sigma(G')\supseteq \sigma(G)'$: $\forall[\sigma(x),\sigma(y)]\in \sigma(G)',[\sigma(x),\sigma(y)]=\sigma[x,y]\in \sigma(G')$
因此 $\sigma(G')=\sigma(G)'\implies \sigma(G^{(2)})=\sigma(G)^{(2)}\implies\sigma(G)^{(k)}=\sigma(G^{(k)})=\sigma(\{e\})=\{\bar e\}$

命题4: 已知群$G$的正规子群$N$
若$N$和$G/N$都是可解群, 则群$G$也是可解群
证明: $G/N是可解群\implies(G/N)^{(k)}=\{N\}$
构造同态$\pi:G\to G/N:g\to gN$
由命题3可知 $\pi(G^{(k)})=\pi(G)^{(k)}=(G/N)^{(k)}=\{N\}$
$\implies\forall a\in G^{(k)},\pi(a)=aN=N\implies a\in N\implies G^{(k)}\subseteq N$
...
$\iff \forall a\neq e,\langle a\rangle=G$, 故群G是素数阶循环群

所有不可交换单群 都是 不可解群
证明: 因为群G不可交换, 所以交换子群$G'\neq\{e\}$
又因为群G是单群, 所以$G'=G$, 因此 $\forall k\in\mathbb{N^*},G^{(k)}=G\neq\{e\}$

有限单群分类定理(Classification of Finite Simple Groups)
1. 素数阶循环群
2. $n\geq5$的交错群$A_n$
3. Lie型单群
4. 26种散在单群
...
次正规子群列的定义(Subgroup Series)
$G=G_0\rhd G_1\rhd G_2\rhd\cdots\rhd G_r=\{e\}$
将$G_0/G_1,G_1/G_2,\cdots,G_{r-1}/G_r$称为因子群组
将非单位元的因子群个数称为该子群列的长度

合成群列的定义(Composition Series)
将每个因子群都是单群的次正规子群列, 称为合成群列

命题1: 每个有限群至少有一个合成群列
证明: 对于任意次正规子群列, 如果存在非单群的因子群$G_i/G_{i+1}$
那么该因子群有非平凡的正规子群 $H/G_{i+1}\lhd G_i/G_{i+1}\implies H\lhd G_i$
因此可以插入H, 得到更长的次正规子群列, 经过有限步, 最终得到合成群列

<br>

Jordan-Holder定理
有限群G的任意两个合成群列的长度相同, 并且其因子群组互相对应同构
$\begin{cases}
...
对应群同态 $\psi:G\to \text{Sym}(X):a\to \psi(a)$
...
已知群G的元素a, 则元素a的中心化子群为$C(a)=\{x\in G|xa=ax\}$
...
称群G的类方程为$|G|=|C(G)|+\sum[Orb(x_i)]$, 其中非中心的代表元$x_i\notin C(G)$
...
将群G的内自同构群记为 $Inn(G)=[\{\sigma_a|\forall a\in G\},\circ]$
...
$Inn(G)\lhd Aut(G)$
1. 运算封闭: $\forall \sigma_a,\sigma_b\in Inn(G),\sigma_a\cdot\sigma_b=\sigma_{ab}\in Inn(G)$
2. 单位元: $\sigma_e(x)=exe^{-1}=x\implies \sigma_e=I$
3. 逆元: $\sigma_{a^{-1}}\circ\sigma_a(x)=a^{-1}(axa^{-1})a=x\implies(\sigma_a)^{-1}=\sigma_{a^{-1}}$
4. 正规: $(\tau\cdot\sigma_a\cdot\tau^{-1})\circ x=\tau[\sigma_a(\tau^{-1}x)]=\tau[a(\tau^{-1}x)a^{-1}]$
$=\tau(a)\cdot[\tau\tau^{-1}x]\cdot\tau(a^{-1})=\tau(a)\cdot x\cdot\tau(a)^{-1}=\sigma_{\tau(a)}(x)$
$\implies\tau\cdot\sigma_a\cdot\tau^{-1}=\sigma_{\tau(a)}\in Inn(G)$

<br>

元素x经过群G作用所得到的 轨道的定义(Orbit)
<元素x的所有能通过G到达的元素>
已知群G在集合X上有一个群作用,
...
$|X|=\sum|Orb(x_i)|=|Fix(G)|+\sum|Orb(x_k)|$

已知$p^k$阶群G, 则$|Fix(G)|\equiv |X|\pmod p$
...
$\sum_{x\in X}|Stab(x)|=\sum_{x\in X}\frac{|G|}{|Orb(x)|}=\sum\limits_{Orb(x)\in X/G}\Sigma_{x\in Orb(x)}\frac{|G|}{|Orb(x)|}=\sum\limits_{Orb(x)\in X/G}|G|=|X/G|\cdot|G|$


## Sylow定理

<引理>
$n=p^ln_0\land \gcd(p,n_0)=1\implies\forall k\le l,C_n^{p^k}=p^{l-k}\_$
...
故至少有一条轨道的p-指数 $\log_p|Orb(S_i)|\le l-k$
$\implies\exists S_i\in X,\log_p|Stab(S_i)|=\log_p\frac{|G|}{|Orb(S_i)|}\ge k$

$Stab(S_i)S_i=S_i\implies\forall s\in S_i,Stab(S_i)\cdot s\subseteq S_i$
因为右陪集与子群具有相同阶, 故$|Stab(S_i)|=|Stab(S_i)s|\le|S_i|=p^k$
综上所述, $\log_p|Stab(S_i)|\ge k\land|Stab(S_i)|\le p^k\implies|Stab(S_i)|=p^k$
...
都有$a\in G$, 使得$k\leq t\implies H\subseteq aPa^{-1}$
...
$\iff\exists a\in G,aP是H在(G/P)_l上的左平移的不动点$
现欲证明 H在$(G/P)_l$上的左平移的 不动点集$\Omega_0$非空
先前已证明 在$p^k$阶群G的群作用中, $|Fix(G)|\equiv |X|\pmod p$
...
正规化子的定义(Normalizer)
<集族上共轭作用的稳定子群>
已知群G, 则其子群H的正规化子为$N_G(H)=\{g\in G|gH=Hg\}$
...
考虑$P_1$在$\Omega$上的共轭作用$P_1\circ P_i=\{\bigcup aP_ia^{-1}|\forall a\in P_1\}$, 其不动点集记为$\Omega_0$
$Q\in\Omega_0\iff\{aQa^{-1}|\forall a\in P_1\}=Q\iff\forall a\in P_1,a\in N_G(Q)\iff P_1\subseteq N_G(Q)$
由于$P_1,Q$是群G的Sylow-p子群, 所以$P_1,Q$也是$N_G(Q)$的Sylow-p子群
由推论可知, Sylow-p正规子群唯一
所以 $Q\lhd N_G(Q)\implies P_1=Q\implies\Omega_0=\{P_1\}\implies|\Omega|\equiv |\Omega_0|=1\pmod p$
又因为$\Omega$为$P_1$在共轭作用下的轨道
所以 $|\Omega|=[G:N_G(P_1)]\implies r|p^ln_0\land(r,p)=1\implies r|n_0$
...
2. 当$r=3$时, 则Sylow-2子群有3个, 即$\Omega=\{P_1,P_2,P_3\}$
    ...
    假如$|\ker(\phi)|=12$, 则$\forall g\in G,g\in\ker(\phi)\implies gP_1g^{-1}=P_1\implies P_1\lhd G$, 与$r=3$矛盾
    因此$2\leq|\ker(\phi)|<12$, 即$\ker(\phi)$是群G的非平凡正规子群

例题: 已知奇素数p, 探究2p阶群G的结构
证明: 群G阶为2p, 故有p阶子群P和2阶子群H
由于素数阶群一定是循环群, 所以$P=\langle a\rangle,H=\langle b\rangle$, 且$|ab|\mid2p$
$[G:P]=\frac{2p}{p}=2\implies (G/P)=\{P,\bar P\}\implies gP=Pg\implies P\lhd G$
...
3. 当$|ab|=p$时, 由于$P=\langle a\rangle\lhd G$
    $\implies P=\langle a\rangle=\langle a\rangle*(ab)^p=\langle a\rangle*ab\cdots ab=b*\langle a\rangle*ab\cdots ab$
    $=\cdots=\langle a\rangle*b^p=\langle a\rangle*b=\bar P$, 得出矛盾
...
<$\widetilde{X^*}$上不可约字构成的群>
由X生成的自由群$F(X)=\widetilde{X^*}/\sim$, 其上运算$\bar w_1\cdot\bar w_2=\overline{w_1w_2}$
1. 运算良定义: $\bar w_1=\bar u_1,\bar w_2=\bar u_2\implies w_1\sim u_1,w_2\sim u_2\implies x_1\to\cdots\to x_l\land y_1\to\cdots\to y_s$
    $\implies (x_1y_1\to x_2y_1\to\cdots\to x_ly_1)\to(x_ly_2\to\cdots\to x_1y_2)\to\cdots\to x_ly_s\implies\overline{w_1w_2}=\overline{u_1u_2}$
...
若小于n的情况成立, 当长度为n时, 若v可约, 则v等价于长度为n-2的短字
而由假设可知该n-2长的短字等价于唯一的不可约字
...
$\implies\phi(\bar w)\phi(\bar u)=f(z_1)\cdots f(z_l)f(y_1)\cdots f(y_s)=\phi(\bar w\bar u)$
...
<任意群=自由群+约束关系>
用构造法证明: 已知群G的一个生成元集Y, 构造与其有双射关系$\tau$的占位符集X
