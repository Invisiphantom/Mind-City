...
已知幺半群$[S,*]$, 及其子集A
由A生成的子幺半群为$\langle A\rangle=\bigcap\{T|A\subseteq T\land T是子幺半群\}$
...
已知幺半群$[S,*]$, 及其子集A
由A生成的子幺半群为$\langle A\rangle=\{x_1^{m_1}x_2^{m_2}\cdots x_k^{m_k}|x_i\in A,m_i\in\mathbb{N},k\in\mathbb{N^+}\}$
...
已知幺半群$[S,\cdot]$和$[T,*]$, 幺半群同态$f:S\to T$
...
已知幺半群$[S,\cdot]$和$[T,*]$, 幺半群同构$f:S\to T$
1. 运算保持: $\forall a,b\in S, f(a\cdot b)=f(a)*f(b)$
2. 单位元保持: $f(e_S)=e_T$
...
<所有同余关系都唯一对应于某个正规子群>
$\implies$: 已知同余关系$a\equiv b$, 欲证明同余类 $\bar1$ 是正规子群
...
4. 正规: $首先证明a\in\bar g\iff a\in g\bar1$
...
4. 正规: $\forall a\in G,n\in\ker(f),f(ana^{-1})=f(a)f(n)f(a^{-1})=f(a)e'f(a)^{-1}=e'$
...
<正规子群 使得商集 能够成为商群>
已知$N\lhd G$, 商群$[G/N,\circ]$, 其上运算$aH\circ bH=(ab)H$
1. 运算良定义: $正规子群\leftrightarrow同余关系$
2. 封闭性: $\forall aN,bN\in G/N,aN\circ bN=(ab)N\in G/N$
...
1. $\varphi(m)=|\mathbb{Z_m^*}|$
2. $\varphi(p)=p-1$
3. $\varphi(p^r)=p^{r-1}(p-1)$
4. $m=m_1m_2\land\gcd(m_1,m_2)=1\implies\varphi(m)=\varphi(m_1)\varphi(m_2)$
5. $m=p_1^{r_1}\cdots p_s^{r_s}\implies\varphi(m)=\varphi(p_1^{r_1})\cdots\varphi(p_s^{r_s})=p_1^{r_1-1}(p_1-1)\cdots p_s^{r_s-1}(p_s-1)$
...
1. 运算保持: $f[(A,U)(B,W)]=f[(AB,UW)]=(AB)(UW)$
    $=(AU)(BW)=f(A,U)f(B,W)$
2. 单射: $f(A,U)=f(B,W)\implies AU=BW$
    $\implies B^{-1}A=WU^{-1}\in SO_3\cap\{I,-I\}=\{I\}\implies A=B,U=W$
3. 满射: $\forall T\in O_3,\begin{cases}
...
1. 运算保持: $f[(h_1,k_1)(h_2,k_2)]=f[(h_1h_2,k_1k_2)]=(h_1h_2)(k_1k_2)$
    $=(h_1k_1)(h_2k_2)=f(h_1,k_1)f(h_2,k_2)$
...
2. $G=G_1G_2\cdots G_n$, 且每个元素的分解方式唯一
...
$a_ia_j=a_ja_i\implies gx_ig^{-1}=(a_1\cdots a_n)x_i(a_1\cdots a_n)^{-1}$
...
<一个群内可交换的元素越多, 换位子种类就越少, 换位子群也就越小>
将所有换位子**所生成**的子群 称为群G的换位子群(导群)
记为$[G,G]=\langle xyx^{-1}y^{-1}|x,y\in G\rangle$, 故群G为交换群, 当且仅当$[G,G]=\{e\}$
正规性: $\forall g\in G,z\in [G,G],gzg^{-1}z^{-1}\in[G,G]$
