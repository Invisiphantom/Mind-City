...
Artin引理: 记$K=\text{Inv}(H)$, 那么$[E:K]\leq|H|$
...
引理2: 记$K=\text{Inv}(H)$, 那么$\text{Gal}(E/K)=H$
...
多项式的伽罗瓦群(Galois Group of Polynomial)
已知$f(x)$的分裂域$E/F$, 则称其伽罗瓦群为$G_F(f)=\text{Gal}(E/F)$
...
引理1: 已知特征为0的域$F$, n阶分圆域$E$, 则$\text{Gal}(E/F)$是交换群
