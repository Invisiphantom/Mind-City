...
则其根群$U\cong[\mathbb{Z_n},+]$, 根自同构群$Aut(U)\cong[\mathbb{Z_n^*},\cdot]$

证明: 因为$char(F)=0$, 所以$\gcd(x^n-1,nx^{n-1})=1$
...
所以$U$是域E的乘法子群, 因此是循环群$U\cong[\mathbb{Z_n},+]$
又因为同构将生成元映到生成元, 并且 $a^k是生成元\iff\gcd(k,n)=1$
所以$Aut(U)\cong[\mathbb{Z_n^*},\cdot]=\{1\leq k<n|\gcd(n,k)=1\}$

------

引理1: 已知零特征域上的多项式$x^n-1$
则其分裂域(分圆域)的伽罗瓦群$\text{Gal}(E/F)<[\mathbb{Z_n^*},*]$
证明: $(\eta r_i)^n=\eta(r_i^n)=1\implies\eta(r_i)\in U\implies\eta|U\in Aut(U)$
可构造映射 $\sigma:G\to Aut(U):\eta\mapsto\eta|U$
1. 运算保持: $\sigma(\eta_1\eta_2)=(\eta_1\eta_2)|U=(\eta_1|U)(\eta_2|U)=\sigma(\eta_1)\sigma(\eta_2)$
2. 单射: $E=F(U)\implies\ker(\sigma)=\{I_E\}$

因此单同态满足$G\cong im(\sigma)<Aut(U)\cong[\mathbb{Z_n^*},*]$

------

引理2: 已知零特征域F含所有n次单位根
则$x^n-a$分裂域的伽罗瓦群是循环群, 其阶数整除n

证明: 记$x^n-a$的分裂域为$E/F$, n次单位根群$U=\{r_1,\cdots,r_n\}\subseteq F$
记$x^n-a$的一个根为$\gamma$, 那么其所有根为$\gamma U=\{\gamma r_1,\cdots,\gamma r_n\}\subseteq E$

$(\eta\gamma)^n=\eta(\gamma^n)=\eta(a)=a\implies\eta(\gamma)\in\gamma U\implies\exists r_i\in U,\eta(\gamma)=\gamma r_i$
可构造映射$\psi_\gamma:G\to U:\eta\mapsto r_i$, 其中$\eta(\gamma)=\gamma r_i$
1. 运算保持: $\eta_1\eta_2(\gamma)=\eta_1(\gamma r_j)=\gamma r_ir_j\implies\psi_\gamma(\eta_1\eta_2)=\psi(\eta_1)\psi(\eta_2)$
2. 单射: $E=F(\gamma U)=F(\gamma)\implies\ker(\psi_\gamma)=\{I_E\}$

因此单同态满足$G\cong im(\psi_\gamma)<U\cong[\mathbb{Z_n},+]\implies G$也是循环群

------

循环扩张的定义(Cyclic Extension)
如果$\text{Gal}(E/F)$是循环群, 则称$E/F$为循环扩张

------

引理3: 已知域F含所有p次单位根
如果$E/F$是$p$次循环扩张, 那么$E=F(d)$, 其中$d^p\in F$
