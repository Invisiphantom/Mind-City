...
对于中间域$L/E/F$, 以下条件等价
(1) $E/F$也是有限正规扩张
(2) $\eta$是域$L$的F-自同构$\implies\eta|E$是域$E$的F-自同构
    $\qquad\iff\forall\eta\in\text{Gal}(L/F),\eta(E)=E$

(1)$\implies$(2): 由命题4和命题3即可知
(2)$\implies$(1): 已知不可约多项式$p(x):U=\{a,\beta_1,\cdots,\beta_k\}$
由命题4可知, 有限正规扩张$L/F\iff$分裂域$L/F$
再由命题2中 分裂域$L/F$的自同构开拓方式可知
有单同态$\{\delta_1,\cdots,\delta_k:h(a)\mapsto h(\beta_i):F(a)\to F(\beta_i)\}$
可开拓成$\{\eta_1,\cdots,\eta_k:a\mapsto\beta_i\}\subseteq\text{Gal}(L/F)$
$a\in E\land\eta_i(a)=\beta_i\land\eta(E)=E\implies U\subseteq E$
...
Artin引理: 已知自同构群$H<\text{Aut}(E)$, 那么$[E:\text{Inv}(H)]\leq|H|$
...
引理2: 已知自同构群$H<\text{Aut}(E)$, 那么$\text{Gal}(E/\text{Inv}(H))=H$
