...
机制: 接收方缓存窗口内的分组
缺陷: 如果窗口长度相对于序号空间过大, 则接收方无法判断
    当前接收的是上一轮的重传分组, 还是新分组
    因此需要满足 窗口长度<=序号空间大小/2


发送方的事件与动作
1. 从上层承接数据: 如果待发送序号位于窗口内, 则打包发送
    否则要么将数据缓存, 要么将其返回给上层等待传输
2. 超时重传: 每个分组都需要拥有其自己的倒计时器
3. 收到ACK: 如果该分组序号在窗口内, 则将其标记为已接收
    如果该分组的序号等于send_base, 则窗口右移到最小未确认序号处
    如果窗口内存在未发送分组, 则发送这些分组
