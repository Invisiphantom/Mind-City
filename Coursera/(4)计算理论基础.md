...
则$L$的Kleene闭包为$L^*=\bigcup_{n=0}^{\infty}L^n=\{\epsilon\}\cup L\cup L^2\cup\cdots$
...
确定性有限自动机的定义(Deterministic Finite Automaton)
5元组$M=(Q,\Sigma,\delta,q_0,F)$称为有限自动机, 其中
1. 状态集: 有限集合$Q$
2. 字母表: 有限集合$\Sigma$
3. 转移函数: $\delta: Q\times\Sigma\to Q$
4. 初始状态: $q_0\in Q$
5. 接受状态集: $F\subseteq Q$

<br>

有限自动机所接受的字符串的定义(String of Finite Automaton)
已知有限自动机$M=(Q,\Sigma,\delta,q_0,F)$,
对于字符串$w=(c_1,c_2,\cdots,c_n)\in\Sigma^*$,
如果存在状态序列$(q_0,q_1,\cdots,q_n), q_i\in Q$, 使得
1. 初始状态: $q_0=q_0$
...
有限自动机所接受的语言的定义(Language of Finite Automaton)
已知有限自动机$M=(Q,\Sigma,\delta,q_0,F)$, 
...
如果存在有限自动机$M$, 使得$L(M)=A$, 则称$A$为正则语言
示例: $\emptyset,\{\epsilon\},\{a\},\Sigma^n,\Sigma^*$都是正则语言
...
正则语言在并运算下封闭
用构造法证明: 已知正则语言$A_1\leftrightarrow M_1,A_2\leftrightarrow M_2$
其中$M_1=(Q_1,\Sigma,\delta_1,q_1,F_1),M_2=(Q_2,\Sigma,\delta_2,q_2,F_2)$
现欲构造有限自动机M, 使得$A_1\cup A_2\leftrightarrow M=(Q,\Sigma,\delta,q_0,F)$
1. 状态集: $Q=Q_1\times Q_2=\{(q_1,q_2)|q_1\in Q_1,q_2\in Q_2\}$
2. 字母表: $\Sigma=\Sigma$
3. 转移函数: $\delta((q_1,q_2),c)=(\delta_1(q_1,c),\delta_2(q_2,c))$
4. 初始状态: $q_0=(q_1,q_2)$
5. 接受状态集: $F=\{(r_1,r_2)|r_1\in F_1\lor r_2\in F_2\}$

<br><br>


幂集$P(Q)=\{A|A\subseteq Q\}$
记$\Sigma_\epsilon = \Sigma\cup\{\epsilon\}$

非确定性有限自动机的定义(Non-deterministic Finite Automaton)
5元组$N=(Q,\Sigma,\delta,q_0,F)$称为有限自动机, 其中
1. 状态集: 有限集合$Q$
2. 字母表: 有限集合$\Sigma$
3. 转移函数: $\delta: Q\times\Sigma_\epsilon\to P(Q)$
4. 初始状态: $q_0\in Q$
5. 接受状态集: $F\subseteq Q$
...
示例: 非确定性有限自动机$N_1=(Q,\Sigma,\delta,q_0,F)$
