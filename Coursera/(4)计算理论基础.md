...
2. $f_1=O(g_1) \land f_2=O(g_2) \iff$
    ...
    $\implies \forall n\geq\max(N_1,N_2), f_1(n)+f_2(n)\leq c_1\cdot g_1(n)+c_2\cdot g_2(n)\leq \max(c_1,c_2)\cdot\max(g_1(n),g_2(n))$
...
字母表上的字符串的定义(String)
已知字母表$\Sigma$, 将非空有限序列$w=(c_1,c_2,\cdots,c_n), c_i\in\Sigma$称为字母表$\Sigma$上的字符串
字符串的符号数称为字符串的长度, 记为$|w|$,
将长度为0的字符串称为空串, 记为$\epsilon=()$

将字母表上长度为$n$的字符串的集合记为$\Sigma^n$
将字母表上所有字符串的集合记为$\Sigma^*=\bigcup_{n=0}^{\infty}\Sigma^n$
...
$[\Sigma^*,\cdot]$是半群, 故满足广义结合律, 可以任意添加括号
...
语言的定义(Language)
已知字母表$\Sigma$, 则$\Sigma^*$的子集称为$\Sigma$上的语言, 记为$L\subseteq\Sigma^*$
需要注意$\emptyset$和$\{\epsilon\}$是两个不同的语言
例如$\emptyset,\Sigma^n,\Sigma^*$都是$\Sigma$上的语言
...
语言的补集的定义(Complement)
已知字母表$\Sigma$, 以及其上的语言$L$, 则$L$的补集为$\overline{L}=\Sigma^*-L$

<br>

语言连接的定义(Concatenation)
已知字母表$\Sigma$, 以及其上的语言$L_1,L_2$, 则$L_1,L_2$的连接为$L_1\cdot L_2=\{xy|x\in L_1,y\in L_2\}$
语言的连接满足结合律, 且$\{\epsilon\}$为单位元, 故$[\forall L,\cdot]$是幺半群
空语言$\emptyset$的作用类似于自然数集中的零元

递归地定义$L_1L_2\cdots L_n=(L_1L_2\cdots L_{n-1})L_n$
$[\forall L,\cdot]$是半群, 故满足广义结合律, 可以任意添加括号
其中规定$L^0=\{\epsilon\}, L^n=\underbrace{L\cdots L}_{n\text{个}}(n\ge1)$

语言的克林闭包的定义(Kleene Closure)
<也就是语言子集所生成的子幺半群>
已知字母表$\Sigma$, 以及其上的语言$L$, 则$L$的Kleene闭包为$L^*=\bigcup_{n=0}^{\infty}L^n=\{\epsilon\}\cup L\cup L^2\cup\cdots$
...
发现$(\ln\ln n)'=\frac{1}{n\ln n}$, 故$\ln\ln n$是$\frac{1}{n\ln n}$的一个原函数
