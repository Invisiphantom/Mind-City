...
![](https://s2.loli.net/2024/09/11/RItWAFYCKOkpiGu.png =500x)
...
![](https://s2.loli.net/2024/09/11/OH9ciTQIUp46eY8.png =500x)
...
证明: 第$\{0,1,\cdots,\log_b n-1\}$层为枝结点层, 第$\log_b n$层为叶子层
逐层累加得 $T(n)=\sum_0^{\log_b n-1}a^if(n/b^i)+\Theta(n^{\log_b a})$

![](https://s2.loli.net/2024/09/12/DmpKdk2qYctUQjy.png =500x)

## 引理2: 幂次散点上的枝结点复杂度

引理2: 已知 $a\geq1,b>1,f(n):\{b^i\}\mapsto\mathbb{R^+}$
枝结点函数 $g(n):\{b^i\}\mapsto\mathbb{R^+}:\sum_0^{\log_bn-1}a^if(n/b^i)$
1. 叶结点: $f(n)=O(n^{\log_ba-\epsilon})\implies g(n)=O(n^{\log_ba})$
2. 均分布: $f(n)=\Theta(n^{\log_ba})\implies g(n)=\Theta(n^{\log_ba}\lg n)$
3. 根结点: $af(n/b)\leq cf(n)\implies g(n)=\Theta(f(n))$
------
...
$\begin{aligned}
    g(n)&=\sum_0^{\log_bn-1}a^if(n/b^i)\leq\sum_0^{\log_b n-1}a^i(n/b^i)^{\log_b a-\epsilon} \\
        ...
        &=n^{\log_ba-\epsilon}\sum_0^{\log_bn-1}b^{i\epsilon}\quad(等比求和) \\
...
$\begin{aligned}
    g(n)&=\sum_0^{\log_bn-1}a^if(n/b^i)\sim\sum_0^{\log_b n-1}a^i(n/b^i)^{\log_ba} \\
        &=n^{\log_ba}\sum_0^{\log_bn-1}a^ib^{-i\log_ba}=n^{\log_ba}\sum_0^{\log_bn-1}a^ia^{-i} \\
...
$\because af(n/b)\leq cf(n)\implies a^if(n/b^i)\leq c^if(n)$
$\begin{aligned}
    \therefore g(n)&=\sum_0^{\log_b n-1}a^if(n/b^i)\leq\sum_0^{\log_b n-1}c^if(n) \\
                &\leq f(n)\sum^\infty c_i=f(n)\frac{1}{1-c}=O(f(n))\end{aligned}$
...
总复杂度 $T(n):\{b^i\}\mapsto\mathbb{R^+}:\begin{cases}\Theta(1) & n=1 \\
            aT(n/b)+f(n) & n=b^i \end{cases}$
1. 叶结点: $f(n)= O(n^{\log_ba-\epsilon})\implies T(n)=\Theta(n^{\log_ba})$
2. 均分布: $f(n)=\Theta(n^{\log_ba})\implies T(n)=\Theta(n^{\log_ba}\log n)$
3. 根结点: $f(n)=\Omega(n^{\log_ba+\epsilon})\land af(n/b)\leq cf(n)\implies T(n)=\Theta(f(n))$
...
叶结点规模 $n_{\lfloor\log_bn\rfloor}\leq \frac{n}{b^{\lfloor\log_bn\rfloor}}+\frac{b}{b-1}<\frac{n}{b^{\log_bn-1}}+\frac{b}{b-1}=b+\frac{b}{b-1}=O(1)$
递归树总复杂度 $T(n)=\sum_0^{\lfloor\log_bn\rfloor-1}a^if(n_i)+\Theta(n^{\log_ba})$
1. 叶结点: $f(n)= O(n^{\log_ba-\epsilon})\implies f(n_i)\leq(n/b^i)^{\log_ba-\epsilon}\overset{(1)}\implies T(n)=\Theta(n^{\log_ba})$
2. 均分布: $f(n)=\Theta(n^{\log_ba})\implies f(n_i)\sim (n/b^i)^{\log_ba}\overset{(2)}\implies T(n)=\Theta(n^{\log_ba}\log n)$
3. 根结点: $f(n)=\Omega(n^{\log_ba+\epsilon})\land af(n/b)\leq cf(n)\overset{(3)}\implies T(n)=\Theta(f(n))$
...
$T(n)=\begin{cases}
    \Theta(1) & n=1 \\[0ex]
    2T(n/2)+\Theta(n) & n>1 \end{cases}\implies\Theta(n\lg n)$

![](https://s2.loli.net/2024/09/05/oiC4XqIfl7OF3dH.png =400x)
