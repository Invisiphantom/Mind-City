...
构造事件: $B_k=\{第一次划分的主元排位为k\}=\{|A_{\leq p}|=k\}\implies P(B_k)=1/n$
...
子问题规模: $T(n-3n/10)=T(7n/10)$
时间复杂度: $T(n)\leq T(n/5)+T(7n/10)+O(n)\implies O(n)$
...
子问题规模: $T(n-2n/7)=T(5n/7)$
时间复杂度: $T(n)\leq T(n/7)+T(5n/7)+O(n)\implies O(n)$
...
子问题规模: $T(n-n/3)=T(2n/3)$
时间复杂度: $T(n)\leq T(n/3)+T(2n/3)+O(n)\implies O(n\lg n)$