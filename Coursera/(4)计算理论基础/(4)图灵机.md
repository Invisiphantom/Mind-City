...
判定器的定义(Decider)
已知非确定性图灵机$M$, 
如果M对于所有可能输入的所有分支都停机, 则称M为判定器

可判定性语言的定义(Decidable Language)
已知字母表$\Sigma$, 其上有语言$A\subseteq\Sigma^*$,
如果存在判定器$M$, 使得$L(M)=A$, 则称$A$为可判定的语言


可判定的语言的示例:
1. $Accept_{DFA}=\{\langle M,w\rangle\mid M\in DFA\land w\in L(M)\}$
2. $Accept_{NFA}=\{\langle M,w\rangle\mid M\in NFA\land w\in L(M)\}$
3. $Accept_{REX}=\{\langle R,w\rangle\mid R\in REX\land w\in L(R)\}$
4. $Empty_{DFA}=\{\langle M\rangle\mid M\in DFA\land L(M)=\emptyset\}$
5. $Equal_{DFA}=\{\langle M,N\rangle\mid M,N\in DFA\land L(M)=L(N)\}$
...
图灵机的可接受问题 是不可判定的
$A_{TM}=\{\langle M,w\rangle\mid M\in TM\land w\in L(M)\}$

先来证明$A_{TM}$是图灵可识别的
构造图灵机U="对于输入$\langle M,w\rangle$:
1. 模拟M对输入字符串w的运行
2. 如果M接受w, 则接受; 如果M循环或拒绝w, 则拒绝"

用反证法证明$A_{TM}$是不可判定的
假设$A_{TM}$是可判定的, 那么存在能够处理循环的判定器H
$H(\langle M,w\rangle)=\begin{cases}
    接受 & 如果M接受w \\
    拒绝 & 如果M循环或拒绝w \end{cases}$

再构造叛逆图灵机D="对于输入$\langle M\rangle$:
1. 用H判定$\langle M,\langle M\rangle\rangle$
2. 如果H接受, 则拒绝; 如果H拒绝, 则接受"

- H接受$\langle M,w\rangle$, 当且仅当M接受w
- H拒绝$\langle M,w\rangle$, 当且仅当M拒绝w或循环
- D拒绝$\langle M\rangle$, 当且仅当M接受$\langle M\rangle$(对角线方法)
- D拒绝$\langle D\rangle$, 当且仅当D接受$\langle D\rangle$, 得出矛盾

![image.png](https://s2.loli.net/2024/04/18/UOW7ALavwDbTunV.png =x200)![image.png](https://s2.loli.net/2024/04/18/jMiZ25b1BtKhuPf.png =x200)
![image.png](https://s2.loli.net/2024/04/18/sqEQITBoueUiP34.png =500x)
...
对该图灵机取逆(将接受状态与拒绝状态交换)
其接受语言$Rej$, 死循环语言$Dead$, 拒绝语言$Acc$
