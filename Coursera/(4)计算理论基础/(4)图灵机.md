...
1. 模拟M对输入字符串w的运行
2. 如果M接受w, 则接受; 如果M循环则循环; 如果M拒绝w, 则拒绝"

用反证法证明$A_{TM}$是不可判定的
假设$A_{TM}$是可判定的, 那么存在能够识别循环的判定器H
$H(M)=\begin{cases}
    接受 & 如果M接受M \\
    拒绝 & 如果M循环或拒绝M \end{cases}$

再构造叛逆图灵机$D(M)=\neg H(M)$
$D(M)=\begin{cases}
    拒绝 & 如果M接受M \\
    接受 & 如果M循环或拒绝M \end{cases}$

因为H是判定器, 所以D也是判定器, 故$D(\forall M)$不会陷入死循环, 要么接受, 要么拒绝
如果$D$接受$D$, 由构造方式可知$D(D)=0$
如果$D$拒绝$D$, 由构造方式可知$D(D)=1$
