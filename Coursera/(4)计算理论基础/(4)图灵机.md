...
1. 状态集: 有限集合$Q$
2. 输入字母表: 有限集合$\Sigma$
3. 纸带字母表: 有限集合$\Gamma$ ($\Sigma\subseteq\Gamma,\square\in\Gamma$)
4. 起始状态: $q_0\in Q$
...
示例: 图灵机对于输入字符串的接受问题 是不可判定的(无法停机)
$A_{TM}=\{\langle M,w\rangle\mid M\in TM\land w\in L(M)\}$
用反证法证明: 假设$A_{TM}$是可判定的, 
那么可构造能够鉴别出死循环的反对角线判定器D
$D(M)=\begin{cases}
    拒绝 & 如果M接受位串(M) \\
    接受 & 如果M死循环或拒绝位串(M) \end{cases}$

![image.png](https://s2.loli.net/2024/04/18/sqEQITBoueUiP34.png =400x)
