...
\left[\frac{\#}{\#\#00011q_1\#}\right]&+ \left[\frac{\#}{\#}\right] \left[\frac{0}{0}\right] \left[\frac{0}{0}\right] \left[\frac{1}{1}\right] \left[\frac{1}{1}\right] \left[\frac{q_1\#}{q_{acc}1\#}\right]\Rightarrow
...
证明: 对于每个或子句$(x_1\lor x_2\lor x_3)$, 构造7-完全团
例如$(\lnot u_2\lor\lnot u_5\lor u_7)\implies V=\{(a,b,c)\neq(1,1,0)\}$
同时如果两个7-完全团中出现顶点部分赋值不一致, 则将其连接
...
$\text{IND\_SET}=_p\text{CLIQUE}=\{图G存在大小为k的完全团\}$
证明: $G=\langle V,E\rangle存在大小为k的独立集\iff\bar G=\langle V,\bar E\rangle存在大小为k的完全团$

------

低度团问题的定义(Low-Degree Clique)
已知图$G=(V,E)$, 判断是否存在一个大小为k的低度团，
团中顶点度数小于等于全图度数的中位数

![output_image.jpeg](https://s2.loli.net/2024/06/08/SoBrDmiPKejwYzc.jpg =500x)


$\text{CLIQUE}\leq_p\text{LDC}=\{图G存在大小为k的低度团\}$
当$k=2$时, 判断2-完全团, 只需判断G是否存在一条边即可
当$k\geq 3$时, 已知图$G=\langle V,E\rangle$, 构建图$G'=G_0+\text{完全二分图V-V}$
现欲证明: 图G有k-完全团$\iff$图G'有k-低度团
$\implies$: 由于完全二分图中每个结点的度数都为n, 故图G'的度数中位数为n
又因为$G_0$中的k-完全团中每个结点度数$\leq n-1$, 所以该k-完全团也是图$G'$的k-低度团
$\impliedby$: 因为$k\geq3$, 所以图G'中的完全二分图部分不含团
所以图G'中的k-低度团只存在于$G_0$中, 即图G存在k-完全团

![image.png](https://s2.loli.net/2024/06/08/EQdUBRZ5APMtS7s.png =200x)![image.png](https://s2.loli.net/2024/06/08/dYeKL1FHzu7b5pr.png =200x)
...
![image.png](https://s2.loli.net/2024/06/07/jvuA9FNiC3WP4VK.png =400x)

![image.png](https://s2.loli.net/2024/06/07/9wt7BMmHeCbaROp.png =400x)

![image.png](https://s2.loli.net/2024/06/07/8PQEmTd7xc1Bt2z.png =400x)
...
![image.png](https://s2.loli.net/2024/06/07/NKMqeHByz3kgSEn.png =400x)
...
并且存在一个在$O(T(n))$时间内输出数$f(n)$二进制表示的图灵机, 例如$n,n^k,2^n$

时间层次定理(Time-Hierarchy Theorum)
已知时间可构造函数f和g, 则$f\log f=o(g)\implies\text{DTIMES}(f)\subsetneq\text{DTIME}(g)$
证明: 稍简单的结论$\text{DTIME}(n)\subsetneq\text{DTIME}(n^{1.5})$
考虑图灵机D:
    在输入x上, 运行高效通用图灵机U模拟$M_x$在x上执行的前$|x|^{1.4}$个步骤
    对于每一步, 如果U输出位b, 则D输入相反答案1-b
    否则, D输出0
根据定义, D在$n^{1.4}$个步骤内停机, 因此由D判定的语言$L\in\text{DTIME}(n^{1.5})$

假设$L\in\text{DTIME(n)}$, 即存在图灵机M和常数c, 
使得对于任意输入位串$x\in\{0,1\}^*$, M在$c|x|$个步骤内停机, 且输出D(x)
