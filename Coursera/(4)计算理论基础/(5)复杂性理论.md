...
| $q_{dead}$ | $(q_{dead},0,R)$ | $(q_{dead},1,R)$ | $(q_{dead},\square,R)$ |
```mermaid
graph LR
    q0 --c=0--> q0
    q0 --c=1--> q1
...
1. 首位骨牌: $[\frac{\#}{\#\#q_00011\#}]$
2. 恒等骨牌: $[\frac{0}{0}],[\frac{1}{1}],[\frac{\square}{\square}],[\frac{\#}{\#}],[\frac{\#}{\square\#}]$
3. 右转移骨牌: $q_0\to[\frac{q_00}{0q_0}],[\frac{q_01}{1q_1}],[\frac{q_0\square}{1q_{acc}}]$
    $q_1\to[\frac{q_10}{0q_{dead}}],[\frac{q_11}{1q_1}],[\frac{q_1\square}{1q_{acc}}]$
    $q_{dead}\to[\frac{q_{dead}0}{0q_{dead}}],[\frac{q_{dead}1}{1q_{dead}}],[\frac{q_{dead}\square}{\square q_{dead}}]$
4. 左转移骨牌: 例如$\delta(q_1,0)=(q_2,1,L)\to[\frac{0q_00}{q_101}],[\frac{1q_00}{q_111}],[\frac{\square q_00}{q_1\square0}]$
    <要添加左侧所有可能出现的字母>
5. 消元骨牌: $[\frac{q_{acc}0}{q_{acc}}],[\frac{q_{acc}1}{q_{acc}}],[\frac{q_{acc}\square}{q_{acc}}]\to吸收右侧$
    $[\frac{0q_{acc}}{q_{acc}}],[\frac{1q_{acc}}{q_{acc}}],[\frac{\square q_{acc}}{q_{acc}}]\to吸收左侧$
6. 结束骨牌: $[\frac{\#q_{acc}\#\#}{\#}]$
