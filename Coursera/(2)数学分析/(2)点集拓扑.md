...
因为$A\sim\mathbb{N^+}$, 所以存在序列$\{x_n\}\leftrightarrow A$
构建子序列$\{x_n\}\cap E$, 对应下标集$\{k_n\}\subseteq\mathbb{N^+}$
因此有等势关系 $E\sim\{k_1,k_2,\cdots\}\sim\mathbb{N^+}$

------

命题2: 可数个可数集的并$\implies$可数集
证明: 可数集的集合$S=\bigcup\{E_n:n\in\mathbb{N^+}\}$
使用对角线方法可遍历序列 $\{x_{11},x_{12},x_{22},\cdots\}$
...
| 拓扑性质 |                                                     |
| -------- | --------------------------------------------------- |
|          | 度量空间$X$, 子集$E\subseteq X$, 点$p\in X$         |
| 邻域     | $N_r^E(p)=\{x\in E\mid d(x,p)<r\}$                  |
| 极限点   | $\forall r>0,\ N_r^E(p)\setminus\{p\}\neq\emptyset$ |
| 孤立点   | $\exists r>0,\ N_r^E(p)\setminus\{p\}=\emptyset$    |
| 内点     | $\exists r>0,N_r(p)\subseteq E$                     |
|          |                                                     |
| 开集     | $E内点集=E$                                         |
| 闭集     | $E极限点集\subseteq E$                              |
| 完全集   | $E极限点集=E$                                       |
...
闭包的定义(Closure)
将集合$E\subseteq X$的极限点集记为$E'$, 闭包为$\bar E=E\cup E'$
1. $E'$是闭集
2. $\bar E=E\cup E'$是闭集
3. $E$是闭集$\iff\bar E=E\cup E'=E$

(1): $(E')'$点的邻域总有$E'$点, 而$E'$点的邻域又总有$E$点
...
证明: $\forall x\in N_r(p)$, 边距 $h=r-d(p,x)$
$\forall x'\in N_h(x)\implies d(p,x')\leq d(p,x)+d(x,x')<(r-h)+h=r$
$\implies x'\in N_r(p)\implies N_h(x)\subseteq N_r(p)\implies x$是内点
...
$\exists r=\min(r_1,\cdots,r_n)>0\implies N_r(x)\subseteq\forall G_i\implies N_r(x)\subseteq H$

(3): 已知$F=\bigcap^I F_a$
$\forall x\in F'\implies\forall r>0,N_r^F(x)\setminus\{x\}\neq\emptyset$
$\implies\forall a\in I,\forall r>0,N_r^{F_a}(x)\setminus\{x\}\neq\emptyset$
$\implies\forall a\in I,x\in(F_a)'\implies\forall a\in I,x\in F_a\implies x\in F$

(4): 已知$K=\bigcup^n F_i$
$\forall x\in K'\implies\forall r>0,N_r^K(x)\setminus\{x\}\neq\emptyset$
$\implies\forall r>0,\exists i\in\{1,\cdots,n\},N_r^{F_i}(x)\setminus\{x\}\neq\emptyset$
逐步取$r:\{1,1/2,1/3,\cdots\}$, 每次取到$\{F_{k_1},F_{k_2},F_{k_3},\cdots\}$
因为个数有限, 所以存在$F_k$取到无限次$\implies x\in F_k'\subseteq F_k\subseteq F$
