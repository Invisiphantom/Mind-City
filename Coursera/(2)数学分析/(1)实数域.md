...
已知有序集$S$, 及其非空有上界子集$E\subseteq S$
称其上确界为$a=\sup E\in S$, 如果满足
1. 上界: $\forall x\in E,x\leq a$
2. 最小上界: $\forall k<a,(\exists x\in E,k<x)$

最大上界的定义(Greatest Upper Bound|Infimum)
已知有序集$S$, 及其非空有下界子集$U\subseteq S$
称其下确界为$b=\inf U\in S$, 如果满足
1. 下界: $\forall x\in U,b\leq x$
2. 最大下界: $\forall h>b,(\exists x\in U,x<h)$

------

最小上界性的定义(Least-Upper-Bound Property)
已知有序集$S$, 称其具有最小上界性
满足 $\forall S\supseteq E有上界\implies\exists\sup E\in S$ 

------

最小上界性$\iff$最大下界性
$\implies$: 已知非空有下界子集 $U\subseteq S$
构造其所有下界的集合 $E=\{x\mid\forall u\in U,x\leq u\}$

由最小上界性可知, $E$有上界$\implies\exists a=\sup E\in S$
现欲证明 集合$E$的上确界$a$ 就是 集合$U$的下确界
1. 下界: $\forall x<a,(\exists e\in E,x<e)\implies x\notin U\implies\forall x\in U,a\leq x\implies a\in E$
2. 最大下界: $\forall e\in E,e\leq a\implies\forall h>a,h\notin E$
...
6. 最小上界: $\delta\subsetneq\gamma\implies(\exists s\in\gamma\setminus\delta)\land(\exists a\in A,s\in a)\implies a\nsubseteq\delta$
