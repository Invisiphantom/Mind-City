...
若$\forall a^i\neq a^j$,则$G=\{a^1,a^2,\cdots,a^{|G|}\}\implies \exist a^k=e$
...
证明: $aH\cap bH\neq\varnothing\implies \exist ah_1=bh_2\implies a=bh_2h_1^{-1}, b=ah_1h_2^{-1}$
...
群由这些划分块所组成
$G=\bigcup_{a\in G}aH$
证明: $\forall g\in G,\exist e\in H,g=ge\in gH\implies G\subseteq\bigcup_{a\in G}aH$
...
由于G被H划分,所以$\forall g\in G,\exist g_i\in G,g\in g_iH$
又由于H被K划分,所以$\forall h\in H,\exist h_j\in H,h\in h_jK$
...
3. 映射满射: $\forall a'\in im(f),\exist a\in G,f(a)=a'\implies \hat{f}(aN)=f(a)=a'$
...
先证明$H\cap N\lhd H$: $\forall h\in H,x\in H\cap N\implies hxh^{-1}\in H\land hxh^{-1}\in N\implies hxh^{-1}\in H\cap N\implies H\cap N\lhd H$

再证明$N\lhd HN$: $\forall hn\in HN,\hat{n}\in N\implies hn\hat{n}(hn)^{-1}=hn\hat{n}n^{-1}h^{-1}\in hNh^{-1}=N\implies N\lhd HN$
...
2. 满射: $\forall y\in G,\exist x=a^{-1}y\implies \phi_a(x)=a(a^{-1}y)=y$
