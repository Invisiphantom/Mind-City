...
证明: $E[XY]=\sum_{(x,y)}xy\cdot p(x,y)$
$X\perp Y\implies p(x,y)=p(x)p(y)\implies E[XY]=\sum xp_X(x)\cdot\sum yp_Y(y)=E[X]E[Y]$
...
期望: $E[X]=\sum x\cdot p_X(x)$
    ...
    $\qquad\qquad\quad\,=N_1\cdot\frac{n!}{N!}\cdot\frac{(N-1)!}{(n-1)!}=nN_1/N$
...
方差: $\text{var}(X)=E[X^2]-(E[X])^2$
    ...
    $\qquad\qquad\qquad=N_1(N_1-1)\cdot\frac{n!}{N!}\cdot\frac{(N-2)!}{(n-2)!}+*$
...
| 概率函数 | $\{\omega\in\Omega:\sum^\infty\omega_i=k\}$ |
| -------- | :-----------------------------------------: |
