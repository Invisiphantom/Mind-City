...
伴随矩阵: $\text{adj}(x)=\begin{bmatrix}\bar a&-b\\\bar b&\bar a\end{bmatrix}$
...
$\implies$: 非零理想只有R $\implies\forall a\in R,1\in(a)\implies\exists xay=1\implies a^{-1}=xy$

已知单环R, 则$\text{char}R=0\lor \text{char}R=p$
证明: 记$k=\text{char}R$, 则$kR=\{0\}$
且$\forall 0<m<k,mR\neq0\implies mR=R$
倘若k是合数, 那么存在分解$k=mn\implies kR=mnR=R$, 得出矛盾
...
证明: $1\in R\land rI\subseteq I\implies RI=I$
$I<R\implies 1\in I\implies RI=R$
...
1. 包含子集: $1\in R\implies\forall a_t\in A,a_t=\sum 1a_t1\in(A)$
...
1. 包含子集: $\forall a\in I,a=a+0\in I+J\implies I\subseteq J$
...
已知满同态$f:R\to R'$, 且子集H包含$\ker(f)$
则H是R的子环(理想), 当且仅当f(H)是R'的子环(理想)
特别地, 若理想$I\supseteq\ker(f)$, 那么存在环同构$p:a+I\leftrightarrow f(a)+f(I)$
1. 加法保持: $p[(a+I)+(b+I)]=p[(a+b)+I]=f(a+b)+f(I)=p(a+I)+p(b+I)$
...
<元素a 映射到 陪集a+I>
$a\to a+I$ $\iff$ $S/(S\cap I)\cong (S+I)/I=S/I$
...
\end{aligned}\right.$
如果任意理想两两互素, 那么方程组有解
$I_i+I_j=R\implies\exists m_j\in I_j,I_i+m_j\ni1\implies M_i=m_1\cdots m_{i-1}m_{i+1}\cdots m_n$
则其解为$x=a_1M_1+\cdots+a_nM_n$, 其中$M_i\equiv\begin{cases}1\pmod{I_i}\\0\pmod{I_j}\end{cases}$

方程组对应的环同态$f:R\to(R/I_1,R/I_2,\cdots,R/I_n):f(x)=(x+I_1,x+I_2,\cdots,x+I_n)$
$\ker(f)=\{x|(x+I_1=I_1)\land(x+I_2=I_2)\land\cdots\land(x+I_n=I_n)\}=\bigcap I_i$
$I_i+I_j=R\implies 对于任意(a_1,a_2,\cdots,a_n)同余方程组都有解\implies f是满同态\implies im(f)=\prod R/I_i$
