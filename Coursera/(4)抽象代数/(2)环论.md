...
3. 乘法单位元: $1=1+0\in S+I$


环同构第二定理(Second Isomorphism Theorem)
<元素a 映射到 陪集a+I>
$a\to a+I$ $\iff$ $S/(S\cap I)\cong (S+I)/I$
...
则可构造方程组解为$x=a_1M_1+\cdots+a_nM_n$, 其中$M_i\equiv\begin{cases}a_i&\pmod{I_i}\\0&\pmod{I_j}\end{cases}$
...
故$I_i+I_j=R\implies R/\bigcap I_i\cong\prod R/I_i$
...
因为R是最大公因数整环, 所以$\exists\gcd(a,p)\implies\exists p=p_0\cdot\gcd(a,p)$
...
$\implies m_i\in R^*\implies p_1\sim q_1\lor p_1\sim q_2\cdots\lor p_1\sim q_n$
