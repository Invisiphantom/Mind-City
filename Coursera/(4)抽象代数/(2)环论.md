...
等价关系: $\frac{a}{b}\sim\frac{c}{d}\iff\exists t\in S,t(rs'-r's)=0\iff ad-bc=0$
...
1. d是a,b的因数: $d|a\land d|b$
1. d是最小的因数: $\forall c\in R,c|a\land c|b\implies cb|ab\land ca|ab$
...
$\forall x\in R\setminus\{0\},x\sim p_1\cdot p_2\cdots p_n\iff x\sim\prod p^{deg_p(x)}$
