...
集合上的变换的定义(Transformation on Set)
已知集合$S$, 则称映射$f:S\to S$为S上的变换
且将其上所有的变换记为$M(S)=\{f|f:S\to S\}$
若集合有限且$|S|=n$, 则称$M(S)$是n阶变换群, 且$|M(S)|=n^n$

变换幺半群的定义(Monoid of Transformations)
已知集合$S$, 则称$[M(S),\circ]$是S上的变换幺半群是幺半群
1. 运算封闭: $\forall f,g\in M(S), f\circ g\in M(S)$
2. 结合律: $\forall f,g,h\in M(S), (f\circ g)\circ h=f\circ(g\circ h)$

<br>

幺半群中的单位的定义(Unit)
幺半群$[S,\cdot]$中的可逆元素称为单位
幺半群中所有可逆元素的集合记为$U(S)$
...
集合上的置换的定义(Permutation on Set)
已知集合$S$, 则称双射$f:S\leftrightarrow S$为S上的置换
可将其记为$f=\begin{pmatrix}1&2&3&\cdots&n\\f(1)&f(2)&f(3)&\cdots&f(n)\end{pmatrix}$
其中映射值两两不同, 即$\forall i\neq j, f(i)\neq f(j)$

对称群的定义(Symmetric Group)
已知集合$S$, 则其上的对称群定义为$sym(S)=\{f|f:S\to S:f是双射\}$
如果$S=\{1,2,\cdots,n\}$, 则称$S_n=sym(S)$为n阶对称群, 且$|S_n|=n!$
如果$S$是有限集, 则可称$sym(S)$为置换群(Permutation Group)
1. 运算封闭: $\forall f,g\in S_n, f\circ g\in S_n$(双射的复合还是双射)
2. 结合律: $\forall f,g,h\in S_n, (f\circ g)\circ h=f\circ(g\circ h)$
3. 单位元: $\exists e\in S_n, \forall f\in S_n, f\circ e=e\circ f=f$
4. 逆元: 映射是双射, 当且仅当其存在逆映射


二面体群的定义(Dihedral Group)
已知正n边形的对称群, 则称其为n阶二面体群, 记为$D_{2n}$
...
元素x的生成子群的定义(Generated Group of Element)
已知$[G,\cdot]$是一个群,且$x\in G$
则元素x的生成子群$\langle x\rangle=\{x^n|n\in\mathbb{Z}\}$也是群
证明: 因为$f:[\mathbb{Z},+]\to [G,\cdot]:f(n)=x^n$是一个群同态,
所以群同态$f$的像$im(f)=\langle x\rangle$也是群

<br>

子集S的生成子群的定义(Generated Group of Set)
...
生成子群是良定义的, 即$\langle x\rangle=\langle\{x\}\rangle$
证明: 因为$\langle x\rangle$是包含了$\{x\}$的子群, 而$\langle\{x\}\rangle$是最小子群, 所以$\langle x\rangle\supseteq\langle\{x\}\rangle$
又因为对于任何子群H, 有$x\in H\implies \forall n\in\mathbb{N^+}, x^n\in H,x^{-n}\in H,x^0=e\in H$
所以有$\langle x\rangle\subseteq\langle\{x\}\rangle$, 综上所述得到$\langle x\rangle=\langle\{x\}\rangle$
