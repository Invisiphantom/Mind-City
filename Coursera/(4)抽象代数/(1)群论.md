...
<在所有G上都映射自身的元素x>
已知群G在集合X上有一个群作用,
则群G的不动点集为$Fix_G(X)=\{x\in X|\forall g\in G,g\circ x=x\}$
不动点的轨道为$Orb(x)=\{x\}$, 稳定子群为$Stab(x)=G$
$|X|=\sum|Orb(x_i)|=|Fix_G(X)|+\sum|Orb(x_j)|$

已知$p^k$阶群G在有限集合X上有一个群作用,
则$|Fix_G(X)|\equiv |X|\pmod p$
证明: 因为$|X|=\sum|Orb(x_i)|=|Fix_G(X)|+\sum|Orb(x_j)|$, 其中$|Orb(x_j)|>1$
又因为$|Orb(x_j)|$是$|G|=p^k$的因子, 所以$|Orb(x_j)|=p^{\geq1}$
故$|X|\equiv |Fix_G(X)|\pmod p$

<br>

伯恩赛德引理(Burnside's Lemma)
<轨道数等于被G中一个元素保持不动的点个数的平均值>
$\sum_{g\in G}|Fix_g(X)|=\sum_{x\in X}|Stab(x)|=r|G|$
证明: 定义$FS=\{(g,x)|g\circ x=x\}\subseteq G\times X$
则有 $Fix_G(X)=FS(g,\cdot)$, $Stab(x)=FS(\cdot,x)$
故 $\sum_{g\in G}|Fix_G(X)|=|FS|=\sum_{x\in X}|Stab(x)|$
