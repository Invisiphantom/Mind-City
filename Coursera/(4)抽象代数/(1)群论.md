...
<能够保持运算良定义的等价关系>
将群G上的等价关系, 称为同余关系
如果$a\equiv a'\land b\equiv b'\implies a*b\equiv a'*b'$
...
2. 单位元: $e\in\bar e$
3. 逆元: $\forall a\in\bar e\implies\overline{a^{-1}}=\bar a^{-1}=\bar e^{-1}=\bar e\implies a^{-1}\in \bar e$
4. 正规: $\bar g=g\bar e\land\bar g=\bar eg\implies g\bar e=\bar eg$
...
4. 运算良定义: $a\equiv a'\land b\equiv b'\land Nb'=bN$
    $\implies b^{-1}Nb'=N\implies b^{-1}a^{-1}a'b'\in N\implies ab\equiv a'b'$
...
$\gcd(k,n)=1\implies|a^k|=\frac{n}{\gcd(k,n)}=n\implies G=\langle x^k\rangle$
...
$|\langle a^d\rangle|=|a^d|=\frac{n}{\gcd(n,d)}=s\implies\langle a^d\rangle$是s阶子群
然后证明唯一性: 任取s阶子群H, 现欲证明H由$a^d$生成
由1知循环群的子群H也是循环群, 不妨设其生成元为$a^k$
于是有$d=\frac{n}{s}=\frac{n}{|a^k|}=\frac{n}{n/\gcd(n,k)}=\gcd(n,k)$
1. $d=\gcd(n,k)\implies d\mid k\implies\langle a^k\rangle\subseteq \langle a^d\rangle$
...
<引理2>
已知有限交换群G, 则群G中元素的最大阶即为$|g|=\text{exp}(G)$
<证明思路>: 已知最大阶元素为$|g|=n$, 假设群G中存在元素$|b|=m\land b^n\neq e$
那么可以构造更大阶的元素, 其阶为$\text{lcm}(n,m)>n=|g|$, 由此得出矛盾
<用反证法证明>: 假设群G中存在元素b, 使得$b^n\neq e\implies|b|=m\nmid n$
那么存在m特有的临界素数p, 使得$p^i|m$, 但是$p^i\nmid n$
则有$m=xp^i, n=yp^j$, 其中$i>j\ge 0$, 并且$\gcd(y,p)=1$
由于$|g^{p^j}|=\frac{n}{\gcd(n,p^j)}=\frac{n}{p^j}=y$, $|b^x|=\frac{m}{\gcd(m,x)}=\frac{m}{x}=p^i$
由引理1可知 $\gcd(y,p^i)=1\implies|g^{p^j}b^x|=yp^i>yp^j=n$, 
这与n是群G的最大阶相矛盾, 所以任意元素b的阶都能整除$|g|$
...
$\implies$: 欲证明逆否命题 $\gcd(m,n)=d\nex\implies\mathbb{Z_m}\times\mathbb{Z_n}$不是循环群
$\gcd(m,n)=d\nex\implies \forall(a,b)\in\mathbb{Z_m}\times\mathbb{Z_n},\underbrace{(a,b)+\cdots+(a,b)}_{lcm(m,n)}=(0,0)$
...
已知群同态$f,g:[G,\cdot]\to[G',*]$
且有群G的一个生成元集$G=\langle S\rangle$
那么 $\forall x\in S,f(x)=g(x)\implies f=g$

自由交换群的定义(Free Abelian Group)
基向量$x_i=(0,\cdots,0,1,0,\cdots,0)$
具有r个自由生成元的自由交换群$\mathbb{Z}^r$
$(n_1,\cdots,n_r)\to a_1^{n_1}\cdots a_r^{n_r}$

自由交换幺半群的定义(Free Abelian Monoid)
具有r个自由生成元的自由交换幺半群$\mathbb{N}^r$

自由幺半群(Free Monoid)
已知字母表$X=\{x_1,x_2,\cdots,x_r\}$
具有r个自由生成元的自由幺半群$\text{FM}^r=X^*=\{e\}\cup X\cup X^2\cup\cdots$

自由群(Free Group)
已知字母表$X=\{x_1,x_2,\cdots,x_r\}$
以及逆元字母表$X'=\{x_1^{-1},x_2^{-1},\cdots,x_r^{-1}\}$
具有r个自由生成元的自由群$\text{FG}^r=\text{FM}^{2r}/\equiv$


示例: 二面体群$[D_n,\circ]=\{I, \sigma, \sigma^2, \cdots, \sigma^{n-1}, \tau, \tau\sigma, \tau\sigma^2, \cdots, \tau\sigma^{n-1}\}$
其中$I$是恒等变换, $\sigma$是顺时针旋转(中心对称), $\tau$是绕轴$(A_1,O)$的镜面翻转(轴对称)
因为$\tau\sigma\tau=\sigma^{-1}$, 故可记为$D_n=\langle\sigma,\tau|\sigma^n=\tau^2=I,\tau\sigma\tau=\sigma^{-1}\rangle$
即$D_n\cong\text{FG}^2/\equiv$, 其中$\equiv$是由$\sigma^n=\tau^2=I,\tau\sigma\tau=\sigma^{-1}$生成的等价关系
...
1. 兼容性: $\forall a,b\in G,x\in G,(ab)\circ x=(ab)x=abxa^{-1}b^{-1}=a(bxb^{-1})a^{-1}=a\circ(b\circ x)$
...
群G在自身的共轭作用为$G\times G\to G:(a,x)\to a\circ x=axa^{-1}$
...
称群G的类方程为$|G|=|C(G)|+\sum[Orb(x_i)]$, 其中代表元$x_i\notin C(G)$
正规子群是共轭类的并集
...
都有$g\in G$, 使得$k\leq l\implies H\subseteq gPg^{-1}$
