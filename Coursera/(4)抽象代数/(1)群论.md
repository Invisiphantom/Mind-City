...
已知非空集合N, 所有从N到自身的双射$f:N\leftrightarrow N$
及其映射复合运算$\circ$构成的群, 称为N上的全变换群, 记作$S_N=[N,\circ]$ 
全变换群$S_N$的 任意子群 称为N的变换群(Transformation Group)

<br>

n阶对称群和n阶置换群的定义(Symmetric Group & Permutation Group)
<对称群, 也可以称为 全置换群>
已知有限集合N, 则称N到自身的双射为N的一个置换(Permutation)
不妨设$N=\{1,2,\cdots,n\}$, 则称$S_N$为n阶对称群, 记作$S_n$
n阶对称群$S_n$的 任意子群 称为n阶置换群(Permutation Group)

<br>

r-轮换的定义(r-Cycle)
已知有限集合N, 如果n元置换$\sigma$将$\left\{\begin{aligned}i_1&\to i_2\\i_2&\to i_3\\&\cdots\\i_r&\to i_1\end{aligned}\right.$
...
任何非单位元的置换都可以分解为不相交的轮换的乘积
<100 Prisoners Problem>

首先证明存在性: 
因为该置换非单位元$f\neq e$, 所以$\exist i_1, f(i_1)\neq i_1$
则有$f(i_1)=i_2, f(i_2)=i_3, \cdots, f(i_r)=i_x$
设$i_x$是第一个重复的元素, 
若$i_x=i_j, j\in\{2, 3, \cdots, r\}$,
则$f(i_{j-1})=i_x\land f(i_r)=i_x$, 
即$i_x$被映两次, 与单射矛盾, 故$i_x=i_1$
于是得到r-轮换$\sigma=(i_1\ i_2\ \cdots\ i_r)$
在$N\setminus\{i_1, i_2, \cdots, i_r\}$上重复上述步骤
最终即可得到$f$的轮换分解式$f=\sigma_1\sigma_2\cdots\sigma_t$

然后证明唯一性:
假设存在两个不同的轮换分解式, 
$f=\sigma_1\sigma_2\cdots\sigma_t=\tau_1\tau_2\cdots\tau_s$
任取变动元素a, 则存在$\sigma_i(a)=\tau_j(a)$
...
生成子群的定义(Generated Group of Set)
...
已知$[G,\cdot]$是一个群,且$S\subseteq G$
则子集S的生成子群$\langle S\rangle=\{x_1^{m_1}x_2^{m_2}\cdots x_k^{m_k}|x_i\in S,m_i\in\mathbb{Z}\}$
...
运算保持: $(\phi_x\circ\phi_y)(a)=\phi_x(\phi_y(a))=x\cdot(y\cdot a)=(x\cdot y)\cdot a=\phi_{x\cdot y}(a)
