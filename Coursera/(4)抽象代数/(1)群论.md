...
定理
半群满足广义结合律, 可以任意添加括号
即 $x_1\cdots x_n\cdot y_1\cdots y_m=(x_1\cdots x_n)(y_1\cdots y_m)$
对m进行归纳证明:
当m=1时, 由定义可知$x_1\cdots x_ny_1=(x_1\cdots x_n)(y_1)$, 满足条件
...
1. 运算封闭: $\forall a,b\in G\implies a^{-1},b^{-1}\in G\implies (a*b)\in S\land(a*b)^{-1}\in S\implies a*b\in G$
2. 结合律: $\forall a,b,c\in G\implies a,b,c\in S\implies (a*b)*c=a*(b*c)$
3. 单位元: $e^{-1}=e\in S\implies e\in G$
...
若运算封闭,单位元保持,逆运算保持, 则称$[T,*]$是一个子群,记为$T<S$
1. 单位元: $e_S\in T$
...
称为f的像(值域)为 $im(f)=\{f(a)|a\in S\}\subseteq T$
...
2. 运算封闭: $\forall f(a),f(b)\in im(f),\exists a,b\in S\implies a\cdot b\in S\implies f(a)*f(b)=f(a\cdot b)\in im(f)$
3. 逆元: $\forall f(a)\in im(f),\exists a\in S\implies f(a)^{-1}=f(a^{-1})\in im(f)$
...
$\iff$ $[\forall x\in\ker(f),f(x)=f(xe_S^{-1})=e_T\implies x=xe_S^{-1}=e_S]$
$\iff$ $\ker(f)=\{e_S\}$
