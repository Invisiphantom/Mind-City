...
同余关系的划分群$G/\equiv$, 有运算$\bar a\cdot\bar b=\overline{ab}$
...
<所有同余关系都唯一对应于某个正规子群>
$\implies$: 已知同余关系$a\equiv b$, 欲证明$N=\bar1$是正规子群
...
$\impliedby$: 已知正规子群$N=\bar1$, 欲证明$a\equiv b:a^{-1}b\in N$是同余关系
1. 自反性: $a^{-1}a=1\in N$
...
<正规子群 使得商集 能够成为商群>
已知$N\lhd G$, 商群$[G/N,\circ]$, 等价关系$a\sim b\iff aN=bN$
1. 运算良定义: $正规子群\leftrightarrow同余关系$
2. 封闭性: $\forall aN,bN\in G/N,aN\circ bN=(ab)N\in G/N$
3. 结合律: $(aN\circ bN)\circ cN=(ab)N\circ cN=(abc)N=aN\circ(b\cdot c)N=aN\circ(bN\circ cN)$
4. 单位元: $aN\circ eN=aN=eN\circ aN$
5. 逆元: $aN\circ a^{-1}N=eN=a^{-1}N\circ aN\implies (aN)^{-1}=a^{-1}N\in G/N$
