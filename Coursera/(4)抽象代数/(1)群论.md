...
引理
已知素数p, 且$n=p^lm\land \gcd(p,m)=1$
则$\forall k\le l,p^{l-k}\mid C_n^{p^k}$, 但 $p^{l-k+1}\nmid C_n^{p^k}$
证明: 已知$C_n^{p^k}=\frac{n(n-1)\cdots(n-j)\cdots(n-p^k+1)}{p^k(p^k-1)\cdots(p^k-j)\cdots 1}$